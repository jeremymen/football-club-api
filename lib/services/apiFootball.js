...
class ApiFootball {
  ...
  getCountries() {
    return new Promise((resolve, reject) => {
      this.http.get('/v2/countries')
        .then(res => {
          return resolve(res.data.api.countries) 
        })
        .catch(err => reject(err))
    })
  }

  getLeagueFromCountry(countryName) {
    return new Promise((resolve, reject) => {
      this.http.get(`/v2/leagues/country/${countryName}/2019`)
        .then(res => {
          return resolve(res.data.api.leagues) 
        })
        .catch(err => reject(err))
    })
  }
  ...
  getTeamsByLeagueId(leagueId) {
    return new Promise((resolve, reject) => {
      this.http.get(`v2/teams/league/${leagueId}`)
        .then(res => {
          return resolve(res.data.api.teams)
        })
        .catch(err => reject(err))
    })
  }
